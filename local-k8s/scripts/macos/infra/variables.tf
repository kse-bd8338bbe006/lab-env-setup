...
variable "network_name" {
  default     = "en0"
  type        = string
  description = "macOS network interface to bridge for VMs (shown by 'multipass networks')"
}

variable "network_gateway" {
  default     = "192.168.50.1"
  type        = string
  description = "Gateway IP for the K8s network (macOS host alias on bridge)"
}
