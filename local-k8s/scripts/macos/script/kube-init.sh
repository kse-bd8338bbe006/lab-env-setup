...
# Get the static IP from k8snet interface (second NIC with static IP), fallback to first IP
export LOCAL_IP=$(ip -4 addr show k8snet 2>/dev/null | grep -oP '(?<=inet\s)192\.168\.50\.\d+' || hostname -I | awk '{print $1}')
