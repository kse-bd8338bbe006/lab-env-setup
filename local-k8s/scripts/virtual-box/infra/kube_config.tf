...
kubectl --kubeconfig "$env:USERPROFILE\.kube\config-virtualbox" config set-context kubernetes-admin@kubernetes --cluster=kubernetes --user=kubernetes-admin
kubectl --kubeconfig "$env:USERPROFILE\.kube\config-virtualbox" config use-context kubernetes-admin@kubernetes
Copy-Item "$env:USERPROFILE\.kube\config-virtualbox" "$env:USERPROFILE\.kube\config" -Force
CMD
