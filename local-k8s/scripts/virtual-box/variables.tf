...
# Network configuration for static IPs (VirtualBox)
# Uses 192.168.56.x (VirtualBox default) to avoid conflict with Hyper-V (192.168.50.x)
variable "network_switch" {
  default     = "VirtualBox Host-Only Ethernet Adapter"
  type        = string
  description = "VirtualBox Host-Only adapter name for VMs (created by setup-network.ps1)"
}
...
locals {
  ...
  # Generated MAC addresses (using 08:00:27 prefix - VirtualBox OUI)
  # Format: 08:00:27:XX:YY:ZZ where XX:YY:ZZ derived from IP suffix
  mac_prefix  = "08:00:27"
